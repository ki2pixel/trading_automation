...
Adaptive Volatility Trend (Passe 1 : L'indicateur Core) sur les nouveaux actifs cryptos qualifiés.
"""
...
def parse_eligible_targets(report_path: Path) -> List[Tuple[str, str]]:
    ...
    for line in section_content.splitlines():
        ...
        for col_idx in [1, 2]:
            ...
            # Regex robuste pour matcher les patterns tf (stratégies)
            ...
            for tf, strats_in_parentheses in matches:
                ...
                if "adaptive_volatility_trend" in strats:
                    tf_clean = tf.strip()
...
def main() -> None:
    ...
    print(f"Trouvé {len(targets)} couples (symbole, timeframe) valides pour adaptive_volatility_trend :")
    ...
    # Définition des paramètres à optimiser (Passe 1 - L'indicateur Core)
    parameters_payload: List[Dict[str, Any]] = [
        ...
        {"name": "preset", "kind": "choice", "values": ["Default"]},
        {"name": "use_rsi_filter", "kind": "choice", "values": [False]},
        {"name": "use_volume_filter", "kind": "choice", "values": [False]},
