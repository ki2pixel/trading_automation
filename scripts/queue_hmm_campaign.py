...
def main():
    ...
    print(f"Trouvé {len(targets)} candidats valides pour hmm_regime_filter.")

    # Définition des paramètres à optimiser alignée sur la Passe 1 historique :
    # - stat_len avec pas de 1 (de 10 à 100)
    # - stick de 0.5 à 0.9 (pas de 0.1)
    parameters_payload = [
        {"name": "obs_len", "kind": "numeric", "start": 5, "end": 30, "step": 1},
        {"name": "stat_len", "kind": "numeric", "start": 10, "end": 100, "step": 1},
        {"name": "mu_k", "kind": "numeric", "start": 0.5, "end": 3.0, "step": 0.1},
        {"name": "stick", "kind": "numeric", "start": 0.5, "end": 0.9, "step": 0.1}
    ]

    # Paramètres bloqués de la Passe 1 historique
    fixed_overrides = {
        "confirm_bars": 2,
        "dom_thresh": 0.5,
    ...
    # Calculer le nombre de runs pour l'intensité bayésienne "deep" (devrait donner exactement 7874)
    ...
    for symbol, tf_str in targets:
        ...
        request_payload = {
            ...
            "workers": 15,
            "min_closed_trades": 50,  # Réglé sur 50 conformément au preset historique
            "max_drawdown_pct": -25.0,
            "min_exposure_pct": 3.0,
            "min_profit_factor": 1.25,
            "max_rows": None,
            "write_best_run": True,
            "score_metric": "return_vs_buy_hold_pct_points",  # Score de sur-performance vs buy and hold
            "score_direction": "max",
