...
HMM Regime Filter (Passe 1) sur les nouveaux actifs cryptos qualifiés.
"""
...
def parse_eligible_targets(report_path: Path) -> List[Tuple[str, str]]:
    ...
    éligibles associés à la stratégie 'hmm_regime_filter'.
    ...
    for line in section_content.splitlines():
        ...
        for col_idx in [1, 2]:
            ...
            # Regex robuste suggérée : r"(\d+min|\d+h|\d+d|\d+m)\s*\(([^)]+)\)"
            ...
            for tf, strats_in_parentheses in matches:
                ...
                if "hmm_regime_filter" in strats:
                    tf_clean = tf.strip()
...
def main() -> None:
    ...
    print(f"Trouvé {len(targets)} couples (symbole, timeframe) valides pour hmm_regime_filter :")
    ...
    # Définition des paramètres à optimiser (Passe 1 - Grille HMM)
    ...
    # Paramètres bloqués (Overrides) conformes à la Passe 1
