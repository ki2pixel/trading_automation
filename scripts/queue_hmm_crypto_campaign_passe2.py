...
HMM Regime Filter (Passe 2 - Filtrage de Régime & Confirmation) sur l'actif crypto qualifié BNBUSDT.
"""
...
def main() -> None:
    print("Préparation de la file d'attente pour la Passe 2 HMM Crypto (Filtrage de Régime & Confirmation)...")

    # Actif validé en Passe 1 et ses paramètres optimaux
    symbol = "bnbusdt"
    tf_str = "60min"
    ...
    # Définition des paramètres à optimiser (confirm_bars de 1 à 5 et dom_thresh de 0.3 à 0.8)
    # Les paramètres HMM issus de la Passe 1 sont bloqués (choice à 1 valeur).
    parameters_payload: List[Dict[str, Any]] = [
        {"name": "obs_len", "kind": "choice", "values": [opt_p1["obs_len"]]},
        {"name": "stat_len", "kind": "choice", "values": [opt_p1["stat_len"]]},
        {"name": "mu_k", "kind": "choice", "values": [opt_p1["mu_k"]]},
        {"name": "stick", "kind": "choice", "values": [opt_p1["stick"]]},
    ...
    print(f"Espace canonique valide : {canonical_iterations} combinaisons réelles (confirm_bars [1-5] x dom_thresh [0.3-0.8]).")
    ...
    print("Enregistrement du job de Passe 2 dans la file d'attente...")
    try:
        ...
        request_payload = {
            ...
            "timeframe": tf_str,
            "optimization_mode": "bayesian",
            "max_iterations": 1000, # Optuna convergera immédiatement car l'espace de recherche réel est minuscule (30 combinaisons)
        ...
        print(f"Erreur lors de l'enregistrement du job Passe 2 : {e}")
        sys.exit(1)
