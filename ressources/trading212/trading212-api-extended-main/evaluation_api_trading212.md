...
Ce rapport évalue la faisabilité et les implications architecturales de la récupération en temps réel des cours des actions européennes via l'API officielle de Trading 212 (Beta) en mode DEMO, pour le suivi de notre portefeuille cible de **37 setups robustes** (portant sur **9 actifs uniques**).
...
Pour suivre simultanément nos **9 actifs uniques** supportant nos **37 setups** d'actifs européens (EVD.DE, GMAB, ZEAL.CO, NVO, NVS, SAP, SHL.DE, AMS.MC, FPE.DE) :
...
1. **Calcul de la consommation de requêtes** :
   ...
   - Nous n'avons pas besoin de faire 9 requêtes séparées pour suivre 9 actifs. **Une seule requête** sur `/equity/portfolio` ou `/equity/positions` permet de récupérer les prix actualisés de nos 9 actifs à la fois.
...
2. **Limitations et Contraintes Opérationnelles** :
   - **Horaires de marché** : Pour ouvrir une micro-position, le marché ciblé (Xetra, Gettex, Bolsa de Madrid) doit être **ouvert**. Si le script tente d'ouvrir une position en dehors des horaires de cotation, l'ordre de marché reste au statut `NEW` (en attente d'ouverture) et la position n'est pas créée. Le prix ne sera disponible qu'à l'ouverture du marché.
   - **Pollution du portefeuille** : La détention de **9 micro-positions** artificielles (au lieu de 37 setups distincts) rend la lecture du portefeuille plus propre mais introduit tout de même des lignes de veille. Les P&L et allocations réels de la stratégie seront masqués ou faussés par ces lignes de "data feeding".
   - **Reset du compte DEMO** : Si le compte DEMO est réinitialisé depuis l'application (pour remettre le solde virtuel à sa valeur par défaut), toutes les positions sont liquidées. Le système de trading doit être capable de détecter cette disparition et de recréer automatiquement les 9 micro-positions.
...
1. **Centralisation du Polling** :
   ...
   - Les 37 instances de stratégies consultent ce cache local au lieu de solliciter l'API de Trading 212, évitant tout risque de blocage par limitation de taux.
...
2. **Séparation Logique par Filtre de Quantité** :
   ...
   - Les positions réellement ouvertes par les signaux de trading des 37 setups utiliseront des tailles de lots normales (calculées via le poids de Kelly, par exemple `0.1` ou `1.0` action minimum).
...
3. **Routine d'Initialisation Automatique (Bootstrap)** :
   - Au démarrage de l'application de paper trading, le script compare la liste des **9 actifs uniques** cibles avec les positions actuellement retournées par l'API.
