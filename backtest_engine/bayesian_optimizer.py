...
def run_bayesian_optimization(
    *,
    ...
    try:
        ...
        from .shm_allocators import allocate_shared_memory_for_strategy

        metadata_dict = allocate_shared_memory_for_strategy(
            strategy=strategy,
            data=data,
        ...
        hma_shm_metadata = metadata_dict["hma_shm_metadata"]
        pmax_shm_metadata = metadata_dict["pmax_shm_metadata"]
        rf_shm_metadata = metadata_dict["rf_shm_metadata"]
        bjorgum_shm_metadata = metadata_dict["bjorgum_shm_metadata"]
    ...
    if run_post_validation and best_row is not None:
        ...
        try:
            ...
            entries_data = rng.choice([True, False], size=(len(price), n_samples), p=[0.1, 0.9])
            exits_data = rng.choice([True, False], size=(len(price), n_samples), p=[0.1, 0.9])
