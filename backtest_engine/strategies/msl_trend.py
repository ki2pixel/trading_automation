...
STRATEGY_FILE = (
    ...
    / "strategy"
    / "msl_trend_strategy.py"
)
_MODULE_NAME = "converted_msl_trend_strategy"
_MODULE: ModuleType | None = None
_MODULE_LOCK = threading.Lock()


@dataclass
class MSLTrendConfigOverrides:
...
def msl_trend_overrides_from_mapping(values: dict[str, object] | None, *, ignore_unknown: bool = True) -> MSLTrendConfigOverrides:
    if not values:
        return MSLTrendConfigOverrides()
    coerced = coerce_strategy_parameters("msl_trend", values, ignore_unknown=ignore_unknown)
    allowed = set(MSLTrendConfigOverrides.__dataclass_fields__.keys())
    return MSLTrendConfigOverrides(**{key: value for key, value in coerced.items() if key in allowed})


def load_msl_trend_overrides_from_config(path: str | Path) -> tuple[MSLTrendConfigOverrides, dict[str, object]]:
    runtime_config = load_strategy_config(path, strategy="msl_trend")
    return msl_trend_overrides_from_mapping(runtime_config.parameters), runtime_config.backtest
...
def _apply_overrides(config: Any, overrides: MSLTrendConfigOverrides) -> Any:
...
def run_msl_trend(
    data: pd.DataFrame,
    symbol: str,
    overrides: MSLTrendConfigOverrides | None = None,
    ...
    overrides = overrides or MSLTrendConfigOverrides()
    ...
    config = module.MSLTrendStrategyConfig()
    config = _apply_overrides(config, overrides)

    bars = _to_strategy_ohlcv(data)
    
    # Vectorized execution
    raw_state, _ = module.run_msl_trend_strategy(bars, config)
