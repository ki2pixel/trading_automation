...
STRATEGY_FILE = (
    ...
    / "strategy"
    / "pivot_retest_strategy.py"
)
_MODULE_NAME = "converted_pivot_retest_strategy"
_MODULE: ModuleType | None = None
_MODULE_LOCK = threading.Lock()


@dataclass
class PivotRetestConfigOverrides:
...
def pivot_retest_overrides_from_mapping(values: dict[str, object] | None, *, ignore_unknown: bool = True) -> PivotRetestConfigOverrides:
    if not values:
        return PivotRetestConfigOverrides()
    coerced = coerce_strategy_parameters("pivot_retest", values, ignore_unknown=ignore_unknown)
    allowed = set(PivotRetestConfigOverrides.__dataclass_fields__.keys())
    return PivotRetestConfigOverrides(**{key: value for key, value in coerced.items() if key in allowed})


def load_pivot_retest_overrides_from_config(path: str | Path) -> tuple[PivotRetestConfigOverrides, dict[str, object]]:
    runtime_config = load_strategy_config(path, strategy="pivot_retest")
    return pivot_retest_overrides_from_mapping(runtime_config.parameters), runtime_config.backtest
...
def _apply_overrides(config: Any, overrides: PivotRetestConfigOverrides) -> Any:
...
def run_pivot_retest(
    data: pd.DataFrame,
    symbol: str,
    overrides: PivotRetestConfigOverrides | None = None,
    ...
    overrides = overrides or PivotRetestConfigOverrides()
    ...
    config = module.PivotRetestStrategyConfig()
    config = _apply_overrides(config, overrides)

    bars = _to_strategy_ohlcv(data)
    
    # Vectorized execution
    raw_state, _ = module.run_pivot_retest_strategy(bars, config)
