"""
backtest_engine/strategies/smart_trader_geometric.py

Wrapper pour la stratégie Smart Trader Geometric dans l'environnement BrokerSimulator.
"""
...
@dataclass
class SmartTraderGeometricConfigOverrides:
    # ── Indicator specific ──
...
def smart_trader_geometric_overrides_from_mapping(
    values: dict[str, object] | None,
    *,
    ignore_unknown: bool = True,
) -> SmartTraderGeometricConfigOverrides:
    if not values:
        return SmartTraderGeometricConfigOverrides()
    coerced = coerce_strategy_parameters(
        "smart_trader_geometric", values, ignore_unknown=ignore_unknown,
    )
    allowed = set(SmartTraderGeometricConfigOverrides.__dataclass_fields__.keys())
    return SmartTraderGeometricConfigOverrides(
        **{key: value for key, value in coerced.items() if key in allowed},
    )

def load_smart_trader_geometric_overrides_from_config(
    path: str | Path,
) -> tuple[SmartTraderGeometricConfigOverrides, dict[str, object]]:
    runtime_config = load_strategy_config(path, strategy="smart_trader_geometric")
    return (
        smart_trader_geometric_overrides_from_mapping(runtime_config.parameters),
...
def run_smart_trader_geometric(
    data: pd.DataFrame,
    symbol: str,
    overrides: SmartTraderGeometricConfigOverrides | None = None,
    ...
    """Run the Smart Trader Geometric strategy backtest."""
    from ._currency_utils import setup_currency_and_fx_provider

    overrides = overrides or SmartTraderGeometricConfigOverrides()
        ...
        if pending_signal is not None and broker_config.execute_on_next_bar:
            if pending_signal == "long" and allow_long:
                    ...
                    if broker.position.signed_quantity < 0:
                        ...
                        broker.fill_order(
                            Order(id=f"exit_short_{order_counter}", side="buy", quantity=qty,
                                  comment="STG cross below (close short)", cost_side="short"),
                            ts, exec_price,
                        )
                    ...
                    if qty > 0:
                        ...
                        broker.fill_order(
                            Order(id=f"entry_long_{order_counter}", side="buy", quantity=qty,
                                  comment="STG entry long", cost_side="long"),
                            ts, exec_price,
                        )
                    ...
                    if broker.position.signed_quantity > 0:
                        ...
                        broker.fill_order(
                            Order(id=f"exit_long_{order_counter}", side="sell", quantity=qty,
                                  comment="STG cross above (close long)", cost_side="long"),
                            ts, exec_price,
                        )
                    qty = broker.calculate_position_size(exec_price, broker.cash, timestamp=ts)
                    qty = broker.normalize_quantity(qty)
                    if qty > 0:
                        ...
                        broker.fill_order(
                            Order(id=f"entry_short_{order_counter}", side="sell", quantity=qty,
                                  comment="STG entry short", cost_side="short"),
                            ts, exec_price,
                        )
                ...
                if broker.position.signed_quantity > 0:
                    ...
                    broker.fill_order(
                        Order(id=f"exit_long_{order_counter}", side="sell", quantity=qty,
                              comment="STG exit long", cost_side="long"),
                        ts, exec_price,
                    )
                ...
                if broker.position.signed_quantity < 0:
                    ...
                    broker.fill_order(
                        Order(id=f"exit_short_{order_counter}", side="buy", quantity=qty,
                              comment="STG exit short", cost_side="short"),
                        ts, exec_price,
                    )
            pending_signal = None
    ...
    if 'Ceil distance' in df.columns:
        ...
        state['Ctr distance'] = df['Ctr distance'].to_numpy()
        state['Flr distance'] = df['Flr distance'].to_numpy()
        state['PnU distance'] = df['PnU distance'].to_numpy()
        state['PnD distance'] = df['PnD distance'].to_numpy()

    if compute_full_metrics:
        metrics, equity_curve = compute_metrics(
            MetricsInput(
                symbol=symbol,
                strategy="smart_trader_geometric",
                initial_capital=initial_capital,
                bars=bars,
                state=state,
                trades=trades,
