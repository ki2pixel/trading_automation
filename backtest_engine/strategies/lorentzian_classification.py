...
def vectorbt_prescan(
    ...
    if stop_requested is not None and stop_requested():
        logger.warning("Pre-scan Lorentzian Classification annulé avant démarrage.")
    ...
    try:
        ...
        for name, default in param_defaults.items():
            ...
            if spec and spec.values:
                if isinstance(default, bool):
                    active_params[name] = [bool(v) for v in spec.values]
        ...
        downsampled = downsample_parameter_grid(
            active_params,
            max_combos=200,  # Aggressive downsampling to prevent OOM
            strategy_name="LorentzianClassification"
        )
        ...
        logger.info(f"Lancement du Pre-Scan Lorentzian Classification ({len(combos)} combos, {total_batches} lots)...")

        for batch_idx in range(total_batches):
            if stop_requested is not None and stop_requested():
                logger.warning("Pre-scan Lorentzian Classification annulé pendant le calcul.")
                _write_prescan_report(output_dir, "cancelled", None, {})
                return parameter_specs
            ...
            lc = LorentzianClassification.run(
                high, low, close,
                **run_kwargs
            )

            long_entries = lc.start_long
            short_entries = lc.start_short
            long_exits = lc.end_long
            short_exits = lc.end_short
        ...
        if top_params:
            ...
            for s in parameter_specs:
                if s.name in keys:
                    ...
                    if isinstance(default_def, bool):
                        new_vals = tuple(v for v in s.values if v in vals)
                    elif isinstance(default_def, int):
            ...
            logger.info(f"Bornes Optuna réduites via VectorBT pour Lorentzian Classification.")
            return new_specs

    except Exception as e:
        logger.warning(f"Erreur Pre-Scan VectorBT Lorentzian Classification: {e}")
