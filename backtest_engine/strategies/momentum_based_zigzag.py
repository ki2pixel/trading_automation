...
STRATEGY_FILE = (
    ...
    / "strategy"
    / "momentum_based_zigzag_strategy.py"
)
_MODULE_NAME = "converted_momentum_based_zigzag_strategy"
_MODULE: ModuleType | None = None
_MODULE_LOCK = threading.Lock()

@dataclass
class MomentumBasedZigZagConfigOverrides:
    rsi_period: int | None = None
    qqe_factor: float | None = None
    rsi_smoothing: int | None = None
    ob: float | None = None
    os: float | None = None
...
def momentum_based_zigzag_overrides_from_mapping(values: dict[str, object] | None, *, ignore_unknown: bool = True) -> MomentumBasedZigZagConfigOverrides:
    if not values:
        return MomentumBasedZigZagConfigOverrides()
    coerced = coerce_strategy_parameters("momentum_based_zigzag", values, ignore_unknown=ignore_unknown)
    allowed = set(MomentumBasedZigZagConfigOverrides.__dataclass_fields__.keys())
    return MomentumBasedZigZagConfigOverrides(**{key: value for key, value in coerced.items() if key in allowed})

def load_momentum_based_zigzag_overrides_from_config(path: str | Path) -> tuple[MomentumBasedZigZagConfigOverrides, dict[str, object]]:
    runtime_config = load_strategy_config(path, strategy="momentum_based_zigzag")
    return momentum_based_zigzag_overrides_from_mapping(runtime_config.parameters), runtime_config.backtest
...
def run_momentum_based_zigzag(
    data: pd.DataFrame,
    symbol: str,
    overrides: MomentumBasedZigZagConfigOverrides | None = None,
    ...
    overrides = overrides or MomentumBasedZigZagConfigOverrides()
    ...
    config = module.MomentumBasedZigZagStrategyConfig()
    ...
    trades = BaseBrokerStrategyRunner._normalize_trades(raw_trades, bars)
    state = BaseBrokerStrategyRunner._build_state_from_broker(broker, bars, timestamps, close_arr)
...
def vectorbt_prescan(
    ...
    if stop_requested is not None and stop_requested():
        logger.warning("Pre-scan Momentum-based ZigZag annulé avant démarrage.")
    ...
    try:
        ...
        downsampled = downsample_parameter_grid(
            active_params,
            max_combos=10000,
            strategy_name="MomentumBasedZigZag"
        )
        ...
        if top_params:
            ...
            for p in top_params:
                # Si top_params contient des tuples à cause de multiples colonnes, sinon c'est un scalaire (si 1 combo)
            ...
            for s in parameter_specs:
                if s.name in keys:
                    ...
                    if isinstance(default_def, int):
                        ...
                        margin = max(0.1, (max_v - min_v) * 0.1)
            ...
            logger.info(f"Bornes Optuna réduites via VectorBT pour Momentum-based ZigZag.")
            return new_specs

    except Exception as e:
        logger.warning(f"Erreur Pre-Scan VectorBT Momentum-based ZigZag: {e}")
