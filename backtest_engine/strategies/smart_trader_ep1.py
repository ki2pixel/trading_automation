"""
backtest_engine/strategies/smart_trader_ep1.py

Wrapper pour la stratégie Smart Trader EP1 dans l'environnement BrokerSimulator.
"""
...
import sys
import os
sys.path.append(os.path.join(os.path.dirname(__file__), '..', '..', 'pine_scripts_convert_to_python', 'strategy'))
try:
...
@dataclass
class SmartTraderEP1ConfigOverrides:
    calc_method: str | None = None
    universal_len: int | None = None
    long_power_min: float | None = None
    short_power_max: float | None = None
    opp_dom_threshold: float | None = None
    max_decay_angle: float | None = None
    sl_buffer_pct: float | None = None
...
def smart_trader_ep1_overrides_from_mapping(
    values: dict[str, object] | None,
    *,
    ignore_unknown: bool = True,
) -> SmartTraderEP1ConfigOverrides:
    if not values:
        return SmartTraderEP1ConfigOverrides()
    coerced = coerce_strategy_parameters(
        "smart_trader_ep1", values, ignore_unknown=ignore_unknown,
    )
    allowed = set(SmartTraderEP1ConfigOverrides.__dataclass_fields__.keys())
    return SmartTraderEP1ConfigOverrides(
        **{key: value for key, value in coerced.items() if key in allowed},
    )

def load_smart_trader_ep1_overrides_from_config(
    path: str | Path,
) -> tuple[SmartTraderEP1ConfigOverrides, dict[str, object]]:
    runtime_config = load_strategy_config(path, strategy="smart_trader_ep1")
    return (
        smart_trader_ep1_overrides_from_mapping(runtime_config.parameters),
...
def _to_strategy_ohlcv(data: pd.DataFrame) -> pd.DataFrame:
    ...
    if "up_volume" in out.columns: cols.append("up_volume")
    if "down_volume" in out.columns: cols.append("down_volume")
...
def run_smart_trader_ep1(
    data: pd.DataFrame,
    symbol: str,
    overrides: SmartTraderEP1ConfigOverrides | None = None,
    ...
    overrides = overrides or SmartTraderEP1ConfigOverrides()
    ...
    config_dict = {}
    for f in SmartTraderEP1Config.__fields__:
        val = getattr(overrides, f, None)
        if val is not None:
            config_dict[f] = val

    config = SmartTraderEP1Config(**config_dict)
    
    df = bars.copy()
    df = add_smart_trader_ep1_features(df, config)

    entries_long = df['long_entry'].fillna(False).to_numpy()
    entries_short = df['short_entry'].fillna(False).to_numpy()
    exits_long = df['long_exit'].fillna(False).to_numpy()
    exits_short = df['short_exit'].fillna(False).to_numpy()
    
    hb_arr = df['strat_hb_price'].to_numpy(dtype=np.float64)
    hs_arr = df['strat_hs_price'].to_numpy(dtype=np.float64)
    
    sl_buf = config.sl_buffer_pct / 100.0
    tp_buf = config.tp_buffer_pct / 100.0

    direction = overrides.trade_direction_mode or "Long & Short"
    allow_long = direction in ("Long & Short", "Long only") and config.allow_long
    allow_short = direction in ("Long & Short", "Short only") and config.allow_short
    ...
    for i in range(n_bars):
        ...
        mark_price = float(close_arr[i])

        # Dynamic TP/SL Evaluation (checked at current bar's close for simplicity)
        if not broker.position.is_flat:
            pos_qty = broker.position.signed_quantity
            hb_p = float(hb_arr[i])
            hs_p = float(hs_arr[i])
            exit_triggered = False
            
            if pos_qty > 0:
                sl_p = hs_p - mark_price * sl_buf if not np.isnan(hs_p) else -1e18
                tp_p = hb_p - mark_price * tp_buf if not np.isnan(hb_p) else 1e18
                if mark_price <= sl_p or mark_price >= tp_p:
                    exit_triggered = True
            elif pos_qty < 0:
                sl_p = hb_p + mark_price * sl_buf if not np.isnan(hb_p) else 1e18
                tp_p = hs_p + mark_price * tp_buf if not np.isnan(hs_p) else -1e18
                if mark_price >= sl_p or mark_price <= tp_p:
                    exit_triggered = True
            ...
            if exit_triggered:
                qty = abs(pos_qty)
                order_counter += 1
                side_str = "sell" if pos_qty > 0 else "buy"
                broker.fill_order(Order(id=f"exit_tpsl_{order_counter}", side=side_str, quantity=qty, comment="EP1 TP/SL Hit"), ts, mark_price)
        ...
        if pending_signal is not None and broker_config.execute_on_next_bar:
            if pending_signal == "long" and allow_long:
                    ...
                    if broker.position.signed_quantity < 0:
                        ...
                        broker.fill_order(
                            Order(id=f"exit_short_{order_counter}", side="buy", quantity=qty,
                                  comment="EP1 reverse (close short)", cost_side="short"),
                            ts, exec_price,
                        )
                    ...
                    if qty > 0:
                        ...
                        broker.fill_order(
                            Order(id=f"entry_long_{order_counter}", side="buy", quantity=qty,
                                  comment="EP1 entry long", cost_side="long"),
                            ts, exec_price,
                        )
                    ...
                    if broker.position.signed_quantity > 0:
                        ...
                        broker.fill_order(
                            Order(id=f"exit_long_{order_counter}", side="sell", quantity=qty,
                                  comment="EP1 reverse (close long)", cost_side="long"),
                            ts, exec_price,
                        )
                    qty = broker.calculate_position_size(exec_price, broker.cash, timestamp=ts)
                    qty = broker.normalize_quantity(qty)
                    if qty > 0:
                        ...
                        broker.fill_order(
                            Order(id=f"entry_short_{order_counter}", side="sell", quantity=qty,
                                  comment="EP1 entry short", cost_side="short"),
                            ts, exec_price,
                        )
                ...
                if broker.position.signed_quantity > 0:
                    ...
                    broker.fill_order(
                        Order(id=f"exit_long_{order_counter}", side="sell", quantity=qty,
                              comment="EP1 exit long", cost_side="long"),
                        ts, exec_price,
                    )
                ...
                if broker.position.signed_quantity < 0:
                    ...
                    broker.fill_order(
                        Order(id=f"exit_short_{order_counter}", side="buy", quantity=qty,
                              comment="EP1 exit short", cost_side="short"),
                        ts, exec_price,
                    )
            pending_signal = None
    ...
    if 'strat_hb_price' in df.columns:
        state['strat_hb_price'] = df['strat_hb_price'].to_numpy()
        state['strat_hs_price'] = df['strat_hs_price'].to_numpy()

    if compute_full_metrics:
        metrics, equity_curve = compute_metrics(
            MetricsInput(
                symbol=symbol,
                strategy="smart_trader_ep1",
                initial_capital=initial_capital,
                bars=bars,
                state=state,
                trades=trades,
