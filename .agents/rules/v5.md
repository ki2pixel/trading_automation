...
- **Reading**: Utilisez `fast_read_file(path="/path/to/file")`.
- **Updating**: Utilisez `edit_file(path="/path/to/file", old_text="...", new_text="...")` pour synchroniser l'état du projet en fin de tâche.
- **Writing**: Utilisez `fast_write_file(path="/path/to/file", content="...")` uniquement pour l'archivage ou l'initialisation des 5 fichiers de base.
- **Listing**: Utilisez `fast_list_directory(path="/home/kidpixel/trading_automation_v2/memory-bank")` pour lister les fichiers disponibles.
- **Project Listing**: Utilisez `fast_get_directory_tree(path="/home/kidpixel/trading_automation_v2/memory-bank")` pour diagnostiquer la connectivité en cas d'échec d'accès.
...
- **Authorized Tools**: `shrimp-task-manager`, `sequentialthinking_tools`, `edit_file`, `fast_read_multiple_files`, `json-query`.
...
- **Editing Standard**: Toute modification de code substantielle doit être réalisée via `edit_file` pour optimiser le contexte.
