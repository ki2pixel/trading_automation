...
- **Sécurisation & Isolation API (SHA256 Failsafe)** : Validation obligatoire du hash SHA256 des clés Bybit/Trading 212 lors de l'initialisation (via `EXPECTED_BYBIT_LIVE_KEY_HASH`, etc.) afin d'interdire tout démarrage avec des clés mélangées.
- **Failsafe Public-Only (Render)** : L'ingestor ou Paper Trader s'exécutant sur Render sans clés privées doit fonctionner en mode public-only. Lever une `ValueError` contrôlée et propre lors de toute tentative d'appel signé Bybit/Trading 212 si les clés API correspondantes sont manquantes (pour éviter les 401).
...
- **Paramètres Redis Robustes** : Assurer la stabilité des sockets Redis Pub/Sub avec options TCP keepalive (`socket_keepalive=True`) et healthcheck interval (`health_check_interval=30`) pour éviter les déconnexions silencieuses du réseau.
