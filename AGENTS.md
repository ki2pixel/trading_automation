...
- **Bufferisation I/O & Accumulateur de Conversion**: Écritures fréquentes (ex: Optuna trials) tamponnées en mémoire et flushées par lots (ex: N=50). Tout SELL Bybit profitable doit verser son PnL net dans `AccumulatorBuffer` (`deposit()`) au sein de la même transaction PostgreSQL pour alimenter le pipeline de conversion USDC→EUR.
